...
│   ├── firefox-perf.js   # тот же замер в реальном Firefox через geckodriver
...
* **Звук** — один `AudioContext`, три gain-канала (`MasterGain`, `MusicGain`, `SfxGain`).
  Все SFX синтезируются осцилляторами и шумом; взмах крыла — это полноценный,
  ни с чем не спутываемый пердёж (две расстроенные пилообразные волны плюс суб-синус
  через lowpass, падающий с 1.1 кГц до 190 Гц, глубокий квадратный LFO по громкости,
  слой band-pass шума для «сырости» и двухступенчатая огибающая 0.42–0.66 с с вторым
  «блатом» на середине; высота, длина и расстройка рандомизируются на каждый взмах). Музыка — процедурная (прогрессия Am–F–C–G, бас,
