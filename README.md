...
| Проверяемость | 48 юнит-тестов логики и 163 интеграционные проверки в headless-браузере |
...
│   ├── browser.test.js   # 152 интеграционные проверки в headless Chromium (CDP)
│   ├── browser.html      # сценарий этих проверок
│   ├── perf.js           # замер кадрового времени по слоям (Chromium)
│   ├── firefox-perf.js   # замер в реальном Firefox через geckodriver + soak-режим
...
Долгие прогоны делаются режимом soak: `SOAK_SECONDS=190 node tests/firefox-perf.js`
печатает по секундам FPS, число кадров тяжелее 33 мс, масштаб рендера и события
(готовность музыки, смены масштаба). Именно он показал, что «лаги в первые две минуты»
создавал мой же контроллер качества: 15 переключений масштаба за 45 секунд, каждое
из которых пересоздаёт canvas и compositing-слой. Сейчас контроллер — аварийная
страховка: не чаще одного изменения в 10 секунд, повышение качества только после
~12 секунд идеальных кадров.
...
node tests/browser.test.js    # 152 проверки в реальном headless Chromium через CDP
node tests/perf.js            # кадровое время по слоям + бюджет кадра (Chromium)
node tests/firefox-perf.js    # то же самое в Firefox: canvas-бэкенд другой,
                              # поэтому Chromium-цифры о Firefox ничего не говорят
...
Переменные окружения: `CHROME`, `CDP_PORT`, `FRAME_BUDGET_MS` (Chromium);
`GECKODRIVER`, `FIREFOX_BIN`, `WDRIVER_PORT`, `FIXED_SCALE` (Firefox, `FIXED_SCALE`
фиксирует масштаб рендера для сравнения).
