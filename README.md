...
│   ├── browser.test.js   # 128 интеграционных проверок в headless Chromium (CDP)
...
* **Звук** — один `AudioContext`, три gain-канала (`MasterGain`, `MusicGain`, `SfxGain`),
  все SFX синтезируются осцилляторами и шумом, музыка генерируется процедурно
  (прогрессия Am–F–C–G, бас, арпеджио, перкуссия) и ускоряется вместе с уровнем сложности.
...
node tests/browser.test.js    # 128 проверок в реальном headless Chromium через CDP
...
падений. Переменные окружения: `CHROME` (путь к браузеру), `CDP_PORT`.
